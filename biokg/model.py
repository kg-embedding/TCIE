...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, evaluator,
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'TCIE']:
    ...
    def TCIE(self, head, relation, tail, mode):
        ...
        head1 = F.normalize(head1, 2, -1)  # 按照某个维度计算范数
        head2 = F.normalize(head1, 2, -1)

        tail1 = F.normalize(tail1, 2, -1)
        ...
        score4 = self.gamma.item() - torch.norm(score, p=1, dim=2)
