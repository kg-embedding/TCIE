...
def override_config(args):
    ...
    应该是checkpoint里保存了模型的参数直接加载文件中的参数
    '''
...
def set_logger(args):
    '''
    Write logs to checkpoint and console 控制台
    '''
...
def log_metrics(mode, step, metrics, writer):
    '''
    Print the evaluation logs、 metrics是个字典
    '''
...
def main(args):
    ...
    "日志保存路径"
    args.save_path = 'config/%s/%s/%s-%s/%s'%(args.dataset, args.model, args.hidden_dim, args.gamma, time.time()) if args.save_path == None else args.save_path
    ...
    if args.cuda:
        net = net.cuda()
    "如果参数是.cuda 使用GPU 有疑问"
    "如果参数是训练，则加载头实体 尾实体训练数据？"
    if args.do_train:
    # Set training dataloader iterator
        ...
        "将从数据库中加载出来的头实体尾实体通过这个类赋值给训练迭代器，应该是一个列表，头实体和尾实体依次"
    ...
    "设置验证集数据加载器 因为为在训练期间可能会评估，感觉是为了收集参数验证的时候用？"
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            "只有参数train_iterator每次在变，指向下一个三元组，其他参数不变"
            log = net.train_step(net, optimizer, train_iterator, args)
            ...
            if step % args.save_checkpoint_steps == 0 and step > 0: # ~ 41 seconds/saving
                ...
                save_model(net, optimizer, save_variable_list, args)
            ...
            if args.do_valid and step % args.valid_steps == 0 and step > 0:
                ...
                metrics = net.test_step(net, valid_triples, args)
    ...
    if args.do_test:
        ...
        metrics = net.test_step(net, test_triples, args)
    ...
    if args.evaluate_train:
        ...
        metrics = net.test_step(net, small_train_triples, args, random_sampling=True)
        log_metrics('Train', step, metrics, writer)
